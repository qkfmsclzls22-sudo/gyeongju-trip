...
## 명소 다양성과 일정 품질 (2026-09-12)

- 행사뿐 아니라 커플 사진·전시, 실내 데이트, 시즌 꽃, 보문·서악·남산의 대안 명소를 계속 찾는다. 플래시백 계림, 라원, 동궁식물원, 엑스포대공원·솔거미술관, 황룡원 외관, 경북천년숲정원, 서악마을을 우선 재확인한다.
- 블로그·SNS·유튜브에서 새 후보를 발견하되 예전 방문 후기를 최신 현황으로 둔갑시키지 않는다. 2024년 커플 후기 https://naezipmaryun2023.tistory.com/70 는 건축 전망·카페 취향 참고만 가능하며 현재 영업정보는 아니다. 2026-09-12 Instagram 및 유튜브 본문은 접근 제한/수집 실패였다.
- 추천 카드 summary에는 어디가 좋은지와 함께 권역, 실내/야외, 주차·입구 혼동, 추천 대상, 방문일 확인 사항 중 유용한 내용을 기록한다. 편집자가 만든 동선 추천은 '기획 추천'으로 사실과 구분한다.
- 라원과 동궁원은 별도 주차장·별도 운영, 동궁원과 동궁과월지는 별개 시설, 플래시백 계림과 도심 계림 숲도 별개 장소다. 엑스포 전시관 간 야외 이동과 온실의 열·습도를 고려한다.
- 봄/가을 꽃은 시즌 참고와 현재 개화 관측을 구분한다. 서악 5월 작약·10월 구절초는 운영자 추천이며 올해 개화/행사일은 별도 확인한다.
- 매일 기존 정보의 휴무·종료·입장마감 변경을 먼저 점검하고 새 명소는 지역·취향의 빈틈을 메울 때 추가한다. 건수를 채우려고 중복 카드나 검증되지 않은 명소를 만들지 않는다.
- 일정 품질 사례에 보문 숙소 커플 1박2일 14시 도착, 월요일·비, 부모님·보행 부담, 17시 귀가를 추가한다. 같은 보문 산책의 낮/밤 반복, 도심→보문 숙소 휴식→도심 야경, 15시 체크인을14시30분 시작, 마지막 날 숙소 재입실은 오류 사례다.
- AI경트는 구조화한 시간표를 lib/itinerary.ts로 검사하고 오류 시 한 번 다시 생성한다. 이는 제한된 시간·중복·권역 검사이며 지도 기반 최단 경로 검증이나 모델 가중치 학습은 아니다. 자동화는 기존처럼 data/now.json만 수정하고 코드 결함은 사례와 함께 보고한다.
